...
public class EffectApproach : EffectMove
{
    ...
    {
        List<BattleTileController> possibleTiles = CellTargeting.ConvertMapRuleToTiles(aoe, targetCell.transform.position)
            .Select(x => x.GetComponent<BattleTileController>()).Where(x => x.unitContents == null).ToList();
